...
export async function login(store) {
  ...
  // Removing (stale) sessions for the account
  if (env.ONLY_ONE_SESSION_PER_ACCOUNT) {
    await removeAllSessions(publisher);
  }
...
export async function logout(session) {
  if (env.ONLY_ONE_SESSION_PER_ACCOUNT) {
    ...
    await removeAllSessions(account);
  } else {
    const sessionRdf = rst.termToString(session);
 ...
 * @async
 * @function
 * @param {NamedNode} account - Represents the account where all the session will be removed from.
 * @returns {undefined} Nothing
 */
async function removeAllSessions(account) {
  ...
  while (batch === undefined || batch.length > 0) {
    ...
    if (batch.length > 0) {
      const sessionsRdf = batch.map((s) => rst.termToString(s)).join('\n');
      await mas.updateSudo(`
      ${env.SPARQL_PREFIXES}
      DELETE {
        GRAPH ?g {
...
async function createSession(session, account, organization) {
  ...
  if (env.SESSION_GRAPH) {
    const sessionGraph = namedNode(env.SESSION_GRAPH);
    await mas.updateSudo(`
      ${env.SPARQL_PREFIXES}
      INSERT DATA {
        GRAPH ${rst.termToString(sessionGraph)} {
...
/*
 * Fetch the account for the given session from the triplestore.
 *
 * @async
 * @function
 * @param {NamedNode} session - Session for which we want the account.
 * @returns {NamedNode} Account linked to the given session.
 */
async function accountForSession(session) {
  const response = await mas.querySudo(`
    ${env.SPARQL_PREFIXES}
    SELECT DISTINCT ?account WHERE {
      ${rst.termToString(session)} muAccount:account ?account .
    }
    LIMIT 1`);
  const parser = new sjp.SparqlJsonParser();
  const parsedResults = parser.parseJsonResults(response);
  return parsedResults[0]?.account;
}
