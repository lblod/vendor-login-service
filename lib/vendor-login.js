...
export async function login(store) {
  ...
  // note the warning in removeAllSessions about this.
  if (env.REMOVE_ALL_SESSIONS_ON_LOGIN) {
...
export async function logout(session) {
  if (env.REMOVE_ALL_SESSIONS_ON_LOGOUT) {
    await removeAllSessions(session);
  }
...
async function verifyPublisherKeyOrganisation(publisher, key, organization) {
  // NOTE: the graph has been remove from this query because we just don't know
  // it. This data is written per organisation graph but a sudo query will get
  // the data.
  ...
  if (env.USE_HASHED_KEY) {
    ...
    try {
      ...
      throw new Error(
        'An error occured while verifying the hashed key for the publisher.',
        { cause: err },
      );
    }
  } else {
    // Keys are NOT hashed. Just check if publisher, organisation and key
    // combination exists.
    const response = await mas.querySudo(`
      ${env.SPARQL_PREFIXES}
...
async function removeAllSessions(session) {
  ...
  // After removing the sessions, it turns out that one vendor (Cevi) does indeed have issues with running its queries.
  // It seems that they do in fact have multiple instances that log in separately and therefore all but one of those instances
  // lose the ability to query our endpoint. Furthermore, we find only 2 sessions in our database for them, both months old, so removing
  // sessions after a certain time would break their flow as well.

  // For now we will just add an environment variable that allows you to remove ALL sessions after login, but we will put a huge warning
  // on this variable as it will break the existing flow for Cevi and possibly other vendors.
  let batch = undefined;
  const parser = new sjp.SparqlJsonParser();
  const safeSession = rst.termToString(session);
